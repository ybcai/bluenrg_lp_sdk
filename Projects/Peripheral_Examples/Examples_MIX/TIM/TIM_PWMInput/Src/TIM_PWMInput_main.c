...
void TimerCaptureCompare_Ch2_Callback()
{
  ...
  uwIC2Value = LL_TIM_IC_GetCaptureCH2(TIM1);

  if (uwIC2Value != 0)
  {
    /* Duty cycle computation */
    uwDutyCycle = (LL_TIM_IC_GetCaptureCH1(TIM1) * 100) / uwIC2Value;

    /* uwFrequency computation
    TIM1 freq  = TIM_PERIPHCLK */
    uwFrequency = TIM_PERIPHCLK  / (1*uwIC2Value);
  }
  else
  {
    uwDutyCycle = 0;
