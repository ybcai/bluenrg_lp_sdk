...
/* Private macro -------------------------------------------------------------*/
#define SPI_READ_CS()           LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_11)
#define SPI_LOW_IRQ()           LL_GPIO_ResetOutputPin(GPIOA, LL_GPIO_PIN_10)
#define SPI_HIGH_IRQ()          LL_GPIO_SetOutputPin(GPIOA, LL_GPIO_PIN_10)

//#define SPI_SENDATA(VAL)        LL_SPI_TransmitData8(SPI1, VAL)
...
static void DMA_Rearm(uint32_t dma_channel, uint32_t buffer, uint32_t size)
{
  ...
  if ((size & 0x01) == 0) {
    ...
    size = (size >> 1) + 1;
    if (dma_channel == DMA_CH_SPI_TX) {
      LL_SPI_SetDMAParity_TX(SPI1, LL_SPI_DMA_PARITY_ODD);
    } else {
  ...
  if (dma_channel == DMA_CH_SPI_TX) {
    LL_DMA_ConfigAddresses(DMA1, dma_channel, buffer, LL_SPI_DMA_GetRegAddr(SPI1), LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
  }
  else {
    LL_DMA_ConfigAddresses(DMA1, dma_channel, LL_SPI_DMA_GetRegAddr(SPI1), buffer, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
  }
#endif
...
void updater_init(void)
{
  ...
  */
  ...
  LL_GPIO_SetAFPin_8_15(GPIOA, LL_GPIO_PIN_14, LL_GPIO_AF_2);
...
#endif
  LL_DMA_SetPeriphRequest(DMA1, DMA_CH_SPI_TX, LL_DMAMUX_REQ_SPI1_TX);
...
#endif
  LL_DMA_SetPeriphRequest(DMA1, DMA_CH_SPI_RX, LL_DMAMUX_REQ_SPI1_RX);
  ...
  */
  ...
  LL_USART_SetBaudRate(USART1, LL_USART_PRESCALER_DIV1, LL_USART_OVERSAMPLING_16, UART_BAUDRATE);
