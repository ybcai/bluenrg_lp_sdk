  ...
  if (hidDeviceStatus() & HID_DEVICE_READY_TO_NOTIFY) {
	...
	keys[0] = 0x00;
      ...
      if (ret != BLE_STATUS_SUCCESS)
        PRINTF("Error during send the report 0x%02x\n", ret);
      keys[0] = 0;
      keys[2] = 0;
      nmbTimes = 0;
      do {
...
void DevicePowerSaveProcedure(void)
{
  ...
  /* If the device is Busy during the HID configuration state machines apply STOP Mode with the wake timer enabled */
  ...
  /* If the device is disconnected or something fails in the connection procedure apply STOP Mode with lowest power save */
