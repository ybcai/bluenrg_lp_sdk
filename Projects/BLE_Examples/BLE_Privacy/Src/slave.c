...
void device_initialization(void)
{
  ...
  //aci_gap_set_authentication_requirement  SC_IS_SUPPORTED
  ...
  status = aci_gap_set_authentication_requirement(BONDING,MITM_PROTECTION_REQUIRED,SC_IS_SUPPORTED,KEYPRESS_IS_NOT_SUPPORTED,0x07,0x10,USE_FIXED_PIN_FOR_PAIRING,FIXED_PIN); 
  if (status != BLE_STATUS_SUCCESS) {
...
void APP_Tick(void)
{
  ...
  else if (APP_FLAG(DO_NOTIFICATIONS) && !APP_FLAG(TX_BUFFER_FULL))
  {
    ...
    if (slaveTimer_expired) 
    {
      ...
      status = aci_gatt_srv_notify(conn_handle, char_handle + 1, 0, 0x2, slave_notification); //TBR???
