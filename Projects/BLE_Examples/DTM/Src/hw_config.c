...
void NVIC_Configuration(void)
{
  ...
  NVIC_SetPriority(USARTx_IRQn, LL_NVIC_MED_PRIORITY);
  ...
  EXTI_InitStruct.Line           = EXTI_CS_PIN;
  ...
  LL_EXTI_ClearInterrupt(EXTI_CS_PIN);
  
  /* Enable the SPI CS Interrupt */
  NVIC_SetPriority(EXTI_CS_IRQn, LL_NVIC_MED_PRIORITY);
  NVIC_EnableIRQ(EXTI_CS_IRQn);
...
void GPIO_Configuration(void)
{
  ...
  GPIO_InitStruct.Alternate = USARTx_GPIO_AF;
  
  LL_GPIO_Init(USARTx_GPIO_PORT, &GPIO_InitStruct);
  ...
  */
  ...
  LL_GPIO_Init(SPIx_GPIO_PORT, &GPIO_InitStruct);  
  
  GPIO_InitStruct.Pin = SPIx_MOSI_PIN | SPIx_MISO_PIN;
  ...
  GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_DOWN;
  LL_GPIO_Init(SPIx_IRQ_PORT, &GPIO_InitStruct); 
...
void UART_Configuration(void)
{
  ...
  LL_USART_ConfigFIFOsThreshold(USARTx_INSTANCE, LL_USART_FIFOTHRESHOLD_1_8, LL_USART_FIFOTHRESHOLD_1_8);
  LL_USART_EnableFIFO(USARTx_INSTANCE);
#ifdef NO_DMA
  LL_USART_EnableIT_RXNE_RXFNE(USARTx_INSTANCE);
#endif
}
...
void SPI_Slave_Configuration(void)
{
  ...
  LL_SPI_Init(SPIx_INSTANCE, &SPI_InitStruct);
  LL_SPI_SetStandard(SPIx_INSTANCE, LL_SPI_PROTOCOL_MOTOROLA);
  LL_SPI_DisableNSSPulseMgt(SPIx_INSTANCE);
  
  LL_SPI_SetNSSMode(SPIx_INSTANCE, LL_SPI_NSS_HARD_INPUT);
#ifdef DMA_16
  LL_SPI_SetRxFIFOThreshold(SPIx_INSTANCE, LL_SPI_RX_FIFO_TH_HALF);
#else
...
{  
...
#endif
  DMA_InitStruct.PeriphRequest = LL_DMAMUX_REQ_SPI1_TX;
  ...
  DMA_InitStruct.MemoryOrM2MDstDataSize = LL_DMA_MDATAALIGN_BYTE;
#endif
  DMA_InitStruct.PeriphRequest = LL_DMAMUX_REQ_SPI1_RX;
  DMA_InitStruct.Priority = LL_DMA_PRIORITY_HIGH;
  LL_DMA_Init(DMA1, DMA_CH_SPI_RX, &DMA_InitStruct);

  /* Enable DMAReq_Tx/Rx */
  LL_SPI_EnableDMAReq_TX(SPIx_INSTANCE);
  LL_SPI_EnableDMAReq_RX(SPIx_INSTANCE);
#endif
  ...
  LL_DMA_ConfigAddresses(DMA1, DMA_CH_UART_RX, LL_USART_DMA_GetRegAddr(USARTx_INSTANCE, LL_USART_DMA_REG_DATA_RECEIVE), (uint32_t)&DMA_RX_Buffer, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
...
void DMA_Rearm(uint32_t dma_channel, uint32_t buffer, uint32_t size)
{
  ...
  LL_DMA_ConfigAddresses(DMA1, dma_channel, buffer, LL_USART_DMA_GetRegAddr(USARTx_INSTANCE, LL_USART_DMA_REG_DATA_TRANSMIT), LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
#endif
  ...
  if ((size & 0x01) == 0) {
    ...
    if (dma_channel == DMA_CH_SPI_TX) {
      LL_SPI_SetDMAParity_TX(SPIx_INSTANCE, LL_SPI_DMA_PARITY_EVEN);
    } else {
      LL_SPI_SetDMAParity_RX(SPIx_INSTANCE, LL_SPI_DMA_PARITY_EVEN);
    }
  } else {
    size = (size >> 1) + 1;
    if (dma_channel == DMA_CH_SPI_TX) {
      LL_SPI_SetDMAParity_TX(SPIx_INSTANCE, LL_SPI_DMA_PARITY_ODD);
    } else {
      LL_SPI_SetDMAParity_RX(SPIx_INSTANCE, LL_SPI_DMA_PARITY_ODD);
    }
  }
...
//#ifdef DMA_16
  LL_SPI_ReceiveData16(SPIx_INSTANCE);  // ISSUE_DMA_16
  LL_SPI_ReceiveData16(SPIx_INSTANCE);
//#endif
  
  /* Rearm the DMA transfer */
  if (dma_channel == DMA_CH_SPI_TX) {
    LL_DMA_ConfigAddresses(DMA1, dma_channel, buffer, LL_SPI_DMA_GetRegAddr(SPIx_INSTANCE), LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
  }
  else {
    LL_DMA_ConfigAddresses(DMA1, dma_channel, LL_SPI_DMA_GetRegAddr(SPIx_INSTANCE), buffer, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
  }
#endif
