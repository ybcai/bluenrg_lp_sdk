...
void USART1_IRQHandler(void)
{
  ...
  while (LL_USART_IsActiveFlag_RXNE_RXFNE(USARTx_INSTANCE))
  {
    /* The Interrupt flag is cleared from the FIFO manager */
    data = LL_USART_ReceiveData8(USARTx_INSTANCE);
    hci_input(&data, 1);
    
    if(LL_USART_IsActiveFlag_ORE(USARTx_INSTANCE)) {
      dbg_overrun_error_flag_usart++;
      LL_USART_RequestRxDataFlush(USARTx_INSTANCE);
...
{
  if(LL_EXTI_IsInterruptPending(EXTI_CS_PIN) == SET) 
  {
     ...
     */
    if(LL_EXTI_GetType(EXTI_CS_PIN) == LL_EXTI_TYPE_LEVEL) {     /* if level sensitive */
      LL_EXTI_SetType(LL_EXTI_TYPE_EDGE, EXTI_CS_PIN);           /* EDGE sensitive */
      DEBUG_NOTES(EDGE_SENSITIVE);
    }
    LL_EXTI_ClearInterrupt(EXTI_CS_PIN);
    
    /* CS pin rising edge - close SPI communication */
    if(LL_EXTI_GetTrigger(EXTI_CS_PIN) == LL_EXTI_TRIGGER_RISING_EDGE && LL_GPIO_IsInputPinSet(SPIx_GPIO_PORT, SPIx_CS_PIN) == 1) {
      LL_GPIO_ResetOutputPin(SPIx_IRQ_PORT, SPIx_IRQ_PIN);
      DEBUG_NOTES(IRQ_FALL);

      DEBUG_NOTES(GPIO_CS_RISING);
      LL_EXTI_SetTrigger(LL_EXTI_TRIGGER_FALLING_EDGE, EXTI_CS_PIN);
    ...
    else if( LL_GPIO_IsInputPinSet(SPIx_GPIO_PORT, SPIx_CS_PIN) == 0) {
      DEBUG_NOTES(GPIO_CS_FALLING);
      LL_EXTI_SetTrigger(LL_EXTI_TRIGGER_RISING_EDGE, EXTI_CS_PIN);
