...
/** @name Options for adv_scan_resp parameter of set_legacy_adv_scan_data
  * @{
  */
#define ADV_DATA                0
...
static tBleStatus allocate_and_set_data_ext(uint8_t Advertising_Handle,
                                            uint8_t Operation, uint8_t Data_Length, uint8_t *Data, uint8_t adv_scan_resp, uint8_t layer);
...
static tBleStatus allocate_and_set_data(uint8_t Advertising_Handle, uint8_t Operation, uint8_t Data_Length, uint8_t *Data, uint8_t adv_scan_resp, uint8_t layer)
{
  return allocate_and_set_data_ext(Advertising_Handle, Operation, Data_Length, Data, adv_scan_resp, layer);
}
...
static uint8_t adv_data_check_param(uint8_t Advertising_Handle, uint8_t Operation, uint8_t Advertising_Data_Length, uint8_t *adv_enabled)
{
  ...
  ret = ll_get_advertising_info(Advertising_Handle, adv_enabled, &adv_event_prop);
...
static tBleStatus set_data_ptr(uint8_t Advertising_Handle,
                                    uint8_t Operation, uint16_t Data_Length, uint8_t *Data, uint8_t scan_resp, uint8_t gap)
{    
  if(!scan_resp){
    // Advertising Data
    if(gap)
                                  ...
                                  uint8_t Operation, uint8_t Data_Length, uint8_t *Data, uint8_t adv_scan_resp, uint8_t layer)
{
  ...
  case UNCHANGED_DATA:
    return set_data_ptr(Advertising_Handle, Operation, Data_Length, Data, adv_scan_resp, layer);
    
  case FIRST_FRAGMENT:
  case COMPLETE_DATA:
    extend = FALSE;
    adv_buff_free_next(Advertising_Handle, adv_scan_resp);  
    break;
  ...
  buffer = adv_buff_alloc(Advertising_Handle, Data_Length, extend, &old_buff_len, adv_scan_resp);
  if(buffer == NULL && Data_Length != 0){
    // Tell the stack that current advertising data has to be cancelled.
    set_data_ptr(Advertising_Handle, COMPLETE_DATA, 0, NULL, adv_scan_resp, layer);
    adv_buff_deactivate_current(Advertising_Handle, adv_scan_resp);
  ...
  if(Operation == LAST_FRAGMENT || Operation == COMPLETE_DATA){
    status = set_data_ptr(Advertising_Handle, COMPLETE_DATA, buff_len, buffer, adv_scan_resp, layer);
    if(status == BLE_STATUS_SUCCESS){
      adv_buff_deactivate_current(Advertising_Handle, adv_scan_resp);
      adv_buff_activate_next(Advertising_Handle, adv_scan_resp);
    }
    else{
