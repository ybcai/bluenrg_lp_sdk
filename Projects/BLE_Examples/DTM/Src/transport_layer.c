...
PowerSaveLevels App_PowerSaveLevel_Check(PowerSaveLevels level)
{
  ...
  if(((SPI_STATE_CHECK(SPI_PROT_SLEEP_STATE) || SPI_STATE_CHECK(SPI_PROT_CONFIGURED_STATE) ) && (fifo_size(&event_fifo) > 0)) || reset_pending) {
    ...
    */   
    LL_EXTI_SetType(LL_EXTI_TYPE_LEVEL, EXTI_CS_PIN);
...
static void transport_layer_send_data(uint8_t *data, uint16_t data_length)
{
  ...
  for (uint16_t i = 0; i < data_length; i++) {
    LL_USART_TransmitData8(USARTx_INSTANCE, data[i]);
    while(LL_USART_IsActiveFlag_TXE_TXFNF(USARTx_INSTANCE) == RESET);
    while(LL_USART_IsActiveFlag_TC(USARTx_INSTANCE) == RESET);
  }
...
//#endif
    ...
    LL_GPIO_SetOutputPin(SPIx_IRQ_PORT, SPIx_IRQ_PIN);       /* Issue the SPI communication request */
  ...
  /* Event queue */
    ...
    if (fifo_get_ptr_var_len_item(&event_fifo, &size, &ptr) == 0) {
      ...
      if(SPI_STATE_CHECK(SPI_PROT_CONFIGURED_EVENT_PEND_STATE) && LL_GPIO_IsInputPinSet(SPIx_GPIO_PORT, SPIx_CS_PIN)) {
        SPI_STATE_TRANSACTION(SPI_PROT_WAITING_HEADER_STATE);        
      }
  ...
  while(SPI_STATE_CHECK(SPI_PROT_WAITING_HEADER_STATE)) {
    ...
    if(tmp_spi_dma_len > 4) {
      ...
      LL_GPIO_ResetOutputPin(SPIx_IRQ_PORT, SPIx_IRQ_PIN); /* Issue the SPI communication request */
...
//        command_fifo_buffer_tmp[rx_buffer_len] = LL_SPI_ReceiveData8(SPIx_INSTANCE);
//        rx_buffer_len++;
//      }
#else
