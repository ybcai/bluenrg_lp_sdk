...
#define HS_STARTUP_TIME_MEASURE 0x02 // ??? [FC] Verify if supported

/* Device Configuration value for struct deviceConfigS */
#define TX_RX_START_STOP_MEASURE 0x03 // ??? [FC] Verify if supported
...
void DTM_SystemInit(void)
{
  ...
  //FLASH->CONFIG |=  CONFIG_IRQ_VECT_FLASH_SECTOR; //TBR???
  ...
  SystemClockConfig(SYSCLK_64M, BLE_SYSCLK_32M);
